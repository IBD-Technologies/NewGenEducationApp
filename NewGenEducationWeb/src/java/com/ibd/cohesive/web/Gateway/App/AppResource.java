...
//                }
          ...
          return errorResponse;
            ...
            JsonObjectBuilder responseBuilder=Json.createObjectBuilder();
            dbg("response -->"+ex.toString());
            if (ex.toString().contains("Token Validation is not success"))
             {
                   this.buildErrorResponse("BSValidationException", new BSValidationException("BS_VAL_100~Token Validation is not success"), content,responseBuilder);
                   JsonObject errorResponse= responseBuilder.build();
                   dbg("errorResponse"+errorResponse.toString());
                   return errorResponse;
