...
var searchResult=[{studentName:'junaid',
					id:'2',
					studentClass:'BS Information Technology'},
					{studentName:'junaid1',
					id:'2',
					studentClass:'BS Information Technology'},
					{studentName:'junaid2',
					...
					{studentName:'junaid4',
					id:'2',
					studentClass:'BE Information Technology'},
					{studentName:'junaid5',
					id:'3',
					studentClass:'BE Information Technology'},
					{studentName:'junaid6',
...
headerTemplate='<div class="row">\n'+
									'<h3 class="">Student search result </h3>\n'+
                                      '<p class="col-orange font-bold">Please double click the record which you want</p>\n'+
                                      '</div>';
                                                                          ;                                      ;
$("#searchResultHeader").empty();                                      
// $("#searchResultHeader").append(headerTemplate);
showResult(searchResult,id,inputValue);
...
function showResult(searchResult,id,searchText){
...
switch(id){
	...
	var resultHeadings='<div class="row">\n'+
                                      ...
                                      '</div>\n'+
                                      '</div>';
$("#searchResultBody").append(resultHeadings);


	for (var i=0; i < searchResult.length; i++) {
		searchResultBodyTemplate='<div id="studentSearchResult_'+i+'" class="row searchResultRow" onclick="hideSearchResult(this.id);">\n'+
                                      '<div class="col-sm-4 col-md-4 col-lg-4 col-xs-4">\n'+
                                          searchResult[i].studentName+'\n'+
                                      '</div>\n'+
                                      ...
                                      '<div class="col-sm-4 col-md-4 col-lg-4 col-xs-4">\n'+
                                        searchResult[i].studentClass+'\n'+
                                      '</div>\n'+
                                      '</div>';

		$("#searchResultBody").append(searchResultBodyTemplate);
        // if (searchResult[i].studentName == searchText) {
        //     for (var j=0; j < searchResult[i].stepper.length; j++) {
        //         if (searchResult[i].stepper[j].Operation == operation) {
        //           for (var k=0; k < searchResult[i].stepper[j].step.length; k++) {
        //             if (searchResult[i].stepper[j].step[k].stepNumber==stepNumber) {
        //                 return searchResult[i].stepper[j].step[k].stepperLabel;
        //             }
        //           }  
        //         }
        //     }
...
function hideSearchResult(id){
	...
	switch(searchId){
		...
		for (var i=0; i < searchResultList.length; i++) {
			if (i==index) {
				$("#StudentName").val(searchResultList[i].studentName);
				$("#studentId").val(searchResultList[i].id);
				console.log(searchResultList[i].studentClass);
				 // $("#studentClass option[value='1']").attr('selected','selected');
				 // $("#studentClass").empty();
				 // console.log($("#studentClass").html());
				 // $("#studentClass").html(' ');
				 // $("#studentClass").append($("<option></option>").attr("value",'h').html('h'));
 				}
		}
	}
}