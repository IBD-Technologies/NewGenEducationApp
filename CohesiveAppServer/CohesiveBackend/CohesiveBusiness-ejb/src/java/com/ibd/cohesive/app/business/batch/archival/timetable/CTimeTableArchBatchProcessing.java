 ...
 */
@Stateless
public class CTimeTableArchBatchProcessing implements ICTimeTableArchBatchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public CTimeTableArchBatchProcessing() throws NamingException {
   ...
   public void processing (String instituteID,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside classTimeTable batch processing");
    ...
    private void sequentialProcessing(String instituteID,String l_businessDate,String batchName)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      try{
           ...
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
           ICTimeTableArchProcessing classArch=inject.getClassTimeTableArchProcessing();
           batchUtil=inject.getBatchUtil(session);
           tc=inject.getTransactionControlService();
           
           Map<String,DBRecord>classAssArchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "CLASS_TIME_TABLE_ARCH_BATCH_STATUS", session, dbSession);
           ...
           for(int i=0;i<classList.size();i++){
              DBRecord classTimeTableRec=classList.get(i);
              String l_standard=classTimeTableRec.getRecord().get(1).trim();
              String l_section=classTimeTableRec.getRecord().get(2).trim();
              String l_status=classTimeTableRec.getRecord().get(4).trim();
               ...
               dbg("l_status"+l_status);
                ...
                if(l_status.equals("F")){
                    ...
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "CLASS_TIME_TABLE_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                    batchUtil.moveClassTimeTableArchRecordToHistory(instituteID,l_standard,l_section,l_businessDate,classTimeTableRec, session, dbSession, inject);
                    tc.commit(session, dbSession);
                }
      ...
      try{
           ...
           for(int i=startIndex;i<=endIndex;i++){
               ...
               if(i<classList.size()){
                ...
                DBRecord stdentTimeTableRec=classList.get(i);
                String l_standard=stdentTimeTableRec.getRecord().get(1).trim();
                String l_section=stdentTimeTableRec.getRecord().get(2).trim();
                String l_status=stdentTimeTableRec.getRecord().get(4).trim();
                ...
                dbg("l_status"+l_status);
                        ...
                        if(l_status.equals("F")){
                            ...
                            String[]l_pkey={instituteID,l_standard,l_section,l_businessDate};
                            dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "CLASS_TIME_TABLE_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                            batchUtil.moveClassTimeTableArchRecordToHistory(instituteID,l_standard,l_section,l_businessDate,stdentTimeTableRec, session, dbSession, inject);
                            tc.commit(session, dbSession);
                        }
     ...
     private void classIdentification(String instituteID,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
           ...
           int tableId=mds.getTableMetaData("BATCH", "CLASS_TIME_TABLE_ARCH_BATCH_STATUS",session).getI_Tableid();
           dbg("tableId"+tableId);
