 ...
 */
@Stateless
public class UProfileArchBatchProcessing implements IUProfileArchBatchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public UProfileArchBatchProcessing() throws NamingException {
   ...
   public void processing (String businessDate,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside userProfile batch processing");
        ...
        if(recordExistence==false||l_eodStatus.equals("F")){
            ...
            userIdentification(businessDate);
        ...
        dbg("end of userProfileArch batch processing");
    ...
    private void sequentialProcessing(String businessDate,String batchName)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      try{
           ...
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
           IUProfileArchProcessing userArch=inject.getUserProfileArchProcessing();
           batchUtil=inject.getBatchUtil(session);
           tc=inject.getTransactionControlService();
           
           Map<String,DBRecord>userAssArchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate, "BATCH", "USER_PROFILE_ARCH_BATCH_STATUS", session, dbSession);
           ...
           for(int i=0;i<userList.size();i++){
              ...
              String l_userID=stdentProfileRec.getRecord().get(0).trim();
              String l_status=stdentProfileRec.getRecord().get(2).trim();
               
               dbg("inside userList iteration");
               dbg("l_status"+l_status);
                ...
                if(l_status.equals("F")){
                    ...
                    String[]l_pkey={l_userID,businessDate};
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate, "BATCH", "USER_PROFILE_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                    batchUtil.moveUserProfileArchRecordToHistory(l_userID,businessDate,stdentProfileRec, session, dbSession, inject);
                    tc.commit(session, dbSession);
                }
                
                userArch.processing(l_userID, businessDate, session);
           }
           ...
           for(int i=startIndex;i<=endIndex;i++){
               ...
               if(i<userList.size()){
                ...
                try{
                  ...
                  Result[j]= (Future<String>)userArch.parallelProcessing(l_userID, businessDate);
                ...
                if( !dispatchFailed){
                     ...
                     WorkDispatch dispatch=new WorkDispatch(l_userID,"WIP");
     ...
     private void userIdentification(String businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       
       try{
           dbg("inside userIdentification");
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
           BusinessService bs=inject.getBusinessService(session);
           ITransactionControlService tc=inject.getTransactionControlService();
           ArrayList<String>userList=bs.getAllUsers( session, dbSession, inject);
           dbg("l_userOfTheGroup"+userList.size());        
           IMetaDataService mds=inject.getMetadataservice();
           int tableId=mds.getTableMetaData("BATCH", "USER_PROFILE_ARCH_BATCH_STATUS",session).getI_Tableid();
           dbg("tableId"+tableId);
           
                for(int j=0;j<userList.size();j++){

                     String l_userID=userList.get(j).trim();
                     dbg("l_userID"+l_userID);
                   
                    try{ 
                     
                     dbts.createRecord(session,"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate, "BATCH", tableId,l_userID,businessDate,"W"," "," ");   
                      tc.commit(session, dbSession);
                     }catch(DBValidationException ex){
