 ...
 */
package com.ibd.cohesive.app.business.batch.archival.assignment;
 ...
 */
@Stateless
public class SAssignmentArchBatchProcessing implements ISAssignmentArchBatchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public SAssignmentArchBatchProcessing() throws NamingException {
   ...
   public void processing (String instituteID,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside studentAssignment batch processing");
    ...
    private void sequentialProcessing(String instituteID,String l_businessDate,String batchName)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      try{
           ...
           dbg("inside s_assignment arch batch--->sequentialProcessing");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
           ISAssignmentArchProcessing studentArch=inject.getStudentAssignmentArchProcessing();
           batchUtil=inject.getBatchUtil(session);
           tc=inject.getTransactionControlService();
           
           Map<String,DBRecord>studentAssArchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "STUDENT_ASSIGNMENT_ARCH_BATCH_STATUS", session, dbSession);
           ...
           for(int i=0;i<studentList.size();i++){
              DBRecord stdentAssignmentRec=studentList.get(i);
              String l_studentID=stdentAssignmentRec.getRecord().get(1).trim();
              String l_status=stdentAssignmentRec.getRecord().get(3).trim();
               ...
               dbg("l_status"+l_status);
                ...
                if(l_status.equals("F")){
                    ...
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "STUDENT_ASSIGNMENT_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                    batchUtil.moveStudentAssignmentArchRecordToHistory(instituteID,l_studentID,l_businessDate,stdentAssignmentRec, session, dbSession, inject);
                    tc.commit(session, dbSession);
                }
...
//     private void studentWiseProcessing(String studentID,String instituteID,String businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
//         BatchUtil batchUtil=null;
//        try{
//             dbg("inside student wise processing");
//             IDBReadBufferService readBuffer=inject.getDBReadBufferService();
//             IBDProperties i_db_properties=session.getCohesiveproperties();
//             IDBTransactionService dbts=inject.getDBTransactionService();
//             BusinessService bs=inject.getBusinessService(session);
...
//             Map<String,DBRecord> studentAssignmentMap=null;
...
//                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate, "BATCH", "STUDENT_ASSIGNMENT_ARCH_BATCH_STATUS", l_primaryKey, column_to_Update,session); 
//                    tc.commit(session, dbSession);
//             
//             
//             
//             int checkerDateColId=mds.getColumnMetaData("STUDENT", "SVW_STUDENT_ASSIGNMENT", "CHECKER_DATE_STAMP", session).getI_ColumnID();
//             dbg("checkerDateColId"+checkerDateColId);       
//             
//             try{
//                 
//                 
//               studentAssignmentMap=  readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"STUDENT"+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID,"STUDENT", "SVW_STUDENT_ASSIGNMENT", session, dbSession, "Yes");
//                 
//                 
//             }catch(DBValidationException ex){
//                 
...
//             if(tableExistence){
//                 
//                Iterator<String>keyIterator= studentAssignmentMap.keySet().iterator();
//                
//                while(keyIterator.hasNext()){
//                    
//                    String key=keyIterator.next();
//                    ArrayList<String>value=studentAssignmentMap.get(key).getRecord();
//                    String checkerDateStamp=value.get(checkerDateColId).trim();
//                    dbg("key"+key);
//                    dbg("checkerDateStamp"+checkerDateStamp);
//                    
//                    if(bs.compareCheckerDate(checkerDateStamp)){
//                        
//                        String[] l_pkey=key.split("~");
//                        
//                        dbg("before delete record call for key"+key);
//                        
//                        dbts.deleteRecord("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"STUDENT"+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID,"STUDENT","SVW_STUDENT_ASSIGNMENT", l_pkey, session);
//                        
...
//        batchUtil.studentAssignmentArchProcessingSuccessHandler(instituteID, businessDate, studentID, inject, session, dbSession);
//        }catch(DBValidationException ex){
//          batchUtil.studentAssignmentArchProcessingErrorHandler(instituteID, businessDate, studentID, ex, inject, session, dbSession);
////        }catch(BSValidationException ex){
////         batchUtil.studentAssignmentArchProcessingErrorHandler(instituteID, businessDate, studentID, ex, inject, session, dbSession);
//        }catch(DBProcessingException ex){
//          dbg(ex);
           ...
           dbg("l_studentOfTheGroup"+studentList.size());        
                ...
                for(int j=0;j<studentList.size();j++){
                    ...
                    try{ 
                     ...
                     dbts.createRecord(session,"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", 154,instituteID,l_studentID,l_businessDate,"W"," "," ");   
                      tc.commit(session, dbSession);
                     }catch(DBValidationException ex){
