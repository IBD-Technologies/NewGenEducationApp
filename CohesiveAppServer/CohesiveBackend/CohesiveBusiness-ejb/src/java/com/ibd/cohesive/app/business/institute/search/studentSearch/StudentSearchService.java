 ...
 * @author DELL
 */
@Remote(IStudentSearchService.class)
@Stateless
public class StudentSearchService implements IStudentSearchService{
    ...
    {     
     ...
     public JsonObject processing(JsonObject requestJson)throws BSProcessingException,BSValidationException,DBValidationException,DBProcessingException{
       ...
       dbg("inside StudentSearchService--->processing");
       dbg("StudentSearchService--->Processing--->I/P--->requestJson"+requestJson.toString());
       ...
       BusinessEJB<IStudentSearchService>studentSearchEJB=new BusinessEJB();
 ...
 return response;
    ...
    private  void buildBOfromReq(JsonObject p_request)throws BSProcessingException,DBProcessingException{
      ...
      RequestBody<StudentSearch> reqBody = new RequestBody<StudentSearch>(); 
    ...
    public void view()throws DBValidationException,DBProcessingException,BSProcessingException,BSValidationException{
        ...
        RequestBody<StudentSearch> reqBody = request.getReqBody();
        ErrorHandler errorHandler=session.getErrorhandler();
        StudentSearch studentSearch =reqBody.get();
    ...
    try{
           ...
           Map<String,List<ArrayList<String>>>classGroup=  l_classMap.values().stream().collect(Collectors.groupingBy(rec->rec.get(1).trim()+rec.get(2).trim()));
           
           Map<String,List<ArrayList<String>>>standardGroup=  l_classMap.values().stream().collect(Collectors.groupingBy(rec->rec.get(1).trim()));
           
           List<ArrayList<String>>filteredRecords=null;
           
           if(classGroup.containsKey(searchFilter)||standardGroup.containsKey(searchFilter)){
               
               dbg("class or standard group contains searchFilter");
               
               filteredRecords=  authorizedRecords.stream().filter(rec->rec.get(2).trim().concat(rec.get(3).trim()).equals(searchFilter)||rec.get(2).trim().equals(searchFilter)).collect(Collectors.toList()); 
               
           }else{
           
              filteredRecords=  authorizedRecords.stream().filter(rec->rec.get(0).trim().contains(searchFilter)||rec.get(1).trim().contains(searchFilter)).collect(Collectors.toList()); 
           ...
           if(searchFilter.equals("")){
               ...
               setRecordInStudentSearch(authorizedRecords);
               
           }else if(filteredRecords!=null&&!filteredRecords.isEmpty()){
               
               setRecordInStudentSearch(filteredRecords);
    ...
    private void setRecordInStudentSearch(List<ArrayList<String>>studentSearchList)throws BSProcessingException,DBValidationException,BSValidationException,DBProcessingException{
        
        try{
           dbg("inside setRecordInStudentSearch"); 
           RequestBody<StudentSearch> reqBody = request.getReqBody();
           StudentSearch studentSearch =reqBody.get();
           BusinessService bs=inject.getBusinessService(session);
           IPDataService pds=inject.getPdataservice();
           ...
           int maxResultSize=Integer.parseInt(session.getCohesiveproperties().getProperty("MAX_SEARCH_RESULTS"));
            int resultSize=0;
               ...
               studentSearch.result=new StudentSearchResult[resultSize];
               
               for(int i=0;i<resultSize;i++){
                   
                   String studentID=filteredSearchList.get(i).get(0).trim();
                   String studentName=filteredSearchList.get(i).get(1).trim();
                   String standard=filteredSearchList.get(i).get(2).trim();
                   String section=filteredSearchList.get(i).get(3).trim();
                   dbg("setRecordInStudentSearch-->studentID"+studentID);
                   dbg("setRecordInStudentSearch-->studentName"+studentName);
                   dbg("setRecordInStudentSearch-->standard"+standard);
                   dbg("setRecordInStudentSearch-->section"+section);
                   studentSearch.result[i]=new StudentSearchResult();
                   studentSearch.result[i].setStudentID(studentID);
                   studentSearch.result[i].setStudentName(studentName);
                   studentSearch.result[i].setStandard(standard);
                   studentSearch.result[i].setSection(section);
               }
            
            
        
        dbg("end of setRecordInStudentSearch");
         }catch(DBValidationException ex){
            throw ex;
        }catch(BSValidationException ex){
            throw ex;    
    ...
    public JsonObject buildJsonResFromBO()throws BSProcessingException{
        ...
        for(int i=0;i<studentSearch.result.length;i++){
            ...
            resultArray.add(Json.createObjectBuilder().add("StudentId", studentSearch.result[i].getStudentID())
                                                      .add("Standard", studentSearch.result[i].getStandard())
                                                      .add("Section", studentSearch.result[i].getSection())
