 ...
 */
@Stateless
public class ClassTimeTableProcessing implements IClassTimeTableProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public ClassTimeTableProcessing() throws NamingException {
    ...
    public void processing (String instituteID,String standard,String section,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside class TimeTable processing");
        ...
        dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "TIMETABLE_BATCH_STATUS", l_pkeyy, column_to_Update,session); 
        tc.commit(session, dbSession);
                
              studentIdentification(instituteID,l_businessDate,standard,section) ;   
              
              String[] l_pkey={"TimeTable"};
         ...
         batchUtil.classTimeTableProcessingSuccessHandler(instituteID, l_businessDate, standard,section, inject, session, dbSession);
        dbg("end of fee processing");
        }catch(DBValidationException ex){
//          tc.rollBack(session, dbSession);
          batchUtil.classTimeTableProcessingErrorHandler(instituteID, l_businessDate, standard,section, ex, inject, session, dbSession);
...
//           tc.rollBack(session, dbSession);
    ...
    public void processing(String instituteID,String feeID,String groupID,String l_businessDate,CohesiveSession session) throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
     ...
     private void sequentialProcessing(String instituteID,String standard,String section,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      ITransactionControlService tc=null;
      try{
           dbg("inside ClassTimeTablebatch -->sequential");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           IBDProperties i_db_properties=session.getCohesiveproperties();
           tc=inject.getTransactionControlService();
           IStudentTimeTableProcessing studentTimeTableProcessing=inject.getStudentTimeTableProcessing();
           IDBTransactionService dbts=inject.getDBTransactionService();
           batchUtil=inject.getBatchUtil(session);
      
            Map<String,DBRecord>studentTimetableBatchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "STUDENT_TIMETABLE_BATCH_STATUS", session, dbSession);
            ...
            dbg("studentList"+studentList.size());
                    ...
                    for(int j=0;j<studentList.size();j++){
                         ...
                         if(l_status.equals("F")){
                            ...
                            batchUtil.moveStudentTimeTableRecordToHistory(instituteID,standard,section,l_studentID,l_businessDate,studentFeeRecord, session, dbSession, inject);
                         }
                         tc.commit(session, dbSession);
                         
                         dbg("before studentTimeTableProcessing call");
                         studentTimeTableProcessing.processing(instituteID,l_studentID,standard,section,l_businessDate,session);
                         dbg("after studentTimeTableProcessing call");
...
//           tc.rollBack(session, dbSession);
   ...
   private void parallelProcessing(String instituteID,String standard,String section,String l_businessDate,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      ITransactionControlService tc=null;
      try{
           dbg("inside timeTableProcessing -->parallel");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IStudentTimeTableProcessing studentTimeTableProcessing=inject.getStudentTimeTableProcessing();
           IDBTransactionService dbts=inject.getDBTransactionService();
           ...
           for(int i=startIndex;i<=endIndex;i++){
               ...
               if(i<studentList.size()){
                ...
                try{
                  ...
                  Result[j]= (Future<String>)studentTimeTableProcessing.parallelProcessing(instituteID, l_studentID, standard, section, l_businessDate);
          ...
          dbg("end of timeTableProcessing -->parellel");
        }catch(DBValidationException ex){
//          tc.rollBack(session, dbSession);
          batchUtil.classTimeTableProcessingErrorHandler(instituteID, l_businessDate, standard,section, ex, inject, session, dbSession);
        }catch(DBProcessingException ex){
           ...
           dbg("l_studentOfTheGroup"+l_studentOfTheGroup.size());        
                ...
                for(int j=0;j<l_studentOfTheGroup.size();j++){
                    ...
                    try{ 
                     ...
                     dbts.createRecord(session,"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", 133,instituteID,p_standard,p_section,l_studentID,l_businessDate,"W"," "," ");   
                      tc.commit(session, dbSession);
                     }catch(DBValidationException ex){
