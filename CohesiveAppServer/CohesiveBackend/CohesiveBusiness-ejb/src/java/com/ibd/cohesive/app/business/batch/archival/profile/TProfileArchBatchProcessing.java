 ...
 */
@Stateless
public class TProfileArchBatchProcessing implements ITProfileArchBatchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public TProfileArchBatchProcessing() throws NamingException {
   ...
   public void processing (String instituteID,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside teacherProfile batch processing");
        ...
        dbg("end of teacherProfileArch batch processing");
    ...
    private void sequentialProcessing(String instituteID,String l_businessDate,String batchName)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      try{
           ...
           dbg("inside profile arch batch--->sequentialProcessing");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
           ITProfileArchProcessing teacherArch=inject.getTeacherProfileArchProcessing();
           batchUtil=inject.getBatchUtil(session);
           tc=inject.getTransactionControlService();
           
           Map<String,DBRecord>teacherAssArchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "TEACHER_PROFILE_ARCH_BATCH_STATUS", session, dbSession);
           ...
           for(int i=0;i<teacherList.size();i++){
              DBRecord teacherProfileRec=teacherList.get(i);
              String l_teacherID=teacherProfileRec.getRecord().get(1).trim();
              String l_status=teacherProfileRec.getRecord().get(3).trim();
               ...
               dbg("l_status"+l_status);
                ...
                if(l_status.equals("F")){
                    ...
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "TEACHER_PROFILE_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                    batchUtil.moveTeacherProfileArchRecordToHistory(instituteID,l_teacherID,l_businessDate,teacherProfileRec, session, dbSession, inject);
                    tc.commit(session, dbSession);
                }
     ...
     private void parallelProcessing(String instituteID,String l_businessDate,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      ITransactionControlService tc=null;
      try{
           dbg("inside teacher profile archival Processing -->parallel");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           ITProfileArchProcessing teacherArch=inject.getTeacherProfileArchProcessing();
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
     ...
     private void teacherIdentification(String instituteID,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
           ...
           int tableId=mds.getTableMetaData("BATCH", "TEACHER_PROFILE_ARCH_BATCH_STATUS",session).getI_Tableid();
           dbg("tableId"+tableId);
