 ...
 */
@Stateless
public class TPayrollArchBatchProcessing implements ITPayrollArchBatchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public TPayrollArchBatchProcessing() throws NamingException {
   ...
   public void processing (String instituteID,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside teacherPayroll batch processing");
        ...
        dbg("end of teacherPayrollArch batch processing");
    ...
    private void sequentialProcessing(String instituteID,String l_businessDate,String batchName)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      try{
           ...
           dbg("inside payroll arch batch--->sequentialProcessing");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
           ITPayrollArchProcessing teacherArch=inject.getTeacherPayrollArchProcessing();
           batchUtil=inject.getBatchUtil(session);
           tc=inject.getTransactionControlService();
           
           Map<String,DBRecord>teacherAssArchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "TEACHER_PAYROLL_ARCH_BATCH_STATUS", session, dbSession);
           ...
           for(int i=0;i<teacherList.size();i++){
              DBRecord teacherPayrollRec=teacherList.get(i);
              String l_teacherID=teacherPayrollRec.getRecord().get(1).trim();
              String l_status=teacherPayrollRec.getRecord().get(3).trim();
               ...
               dbg("l_status"+l_status);
                ...
                if(l_status.equals("F")){
                    ...
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "TEACHER_PAYROLL_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                    batchUtil.moveTeacherPayrollArchRecordToHistory(instituteID,l_teacherID,l_businessDate,teacherPayrollRec, session, dbSession, inject);
                    tc.commit(session, dbSession);
                }
                
                teacherArch.processing(l_teacherID, instituteID, l_businessDate, session);
           }
          dbg("end of payroll batch--->sequentialProcessing");  
         batchUtil.batchSucessHandler(instituteID, l_businessDate, batchName, inject, session, dbSession);
        }catch(DBValidationException ex){
          batchUtil.batchErrorHandler(instituteID, l_businessDate, ex, batchName, inject, session, dbSession);
        }catch(BSValidationException ex){
     ...
     private void parallelProcessing(String instituteID,String l_businessDate,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      ITransactionControlService tc=null;
      try{
           dbg("inside teacher payroll archival Processing -->parallel");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           ITPayrollArchProcessing teacherArch=inject.getTeacherPayrollArchProcessing();
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
     ...
     private void teacherIdentification(String instituteID,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
           ...
           int tableId=mds.getTableMetaData("BATCH", "TEACHER_PAYROLL_ARCH_BATCH_STATUS",session).getI_Tableid();
           dbg("tableId"+tableId);
