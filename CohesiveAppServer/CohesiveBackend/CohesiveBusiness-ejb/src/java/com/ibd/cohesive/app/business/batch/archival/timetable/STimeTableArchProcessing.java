 ...
 */
@Stateless
public class STimeTableArchProcessing implements ISTimeTableArchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public STimeTableArchProcessing() throws NamingException {
  ...
  public void processing (String studentID,String instituteID,String businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
             ...
             dbg("student timetable arch processing");
             String currentDate=bs.getCurrentDate();
             int tableId=mds.getTableMetaData("STUDENT", "SVW_STUDENT_TIMETABLE_MASTER", session).getI_Tableid();
             Map<String,DBRecord> studentTimeTableMap=null;
             boolean tableExistence=true;
                    ...
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate, "BATCH", "STUDENT_TIME_TABLE_ARCH_BATCH_STATUS", l_primaryKey, column_to_Update,session); 
                    tc.commit(session, dbSession);
             
             
             
             int checkerDateColId=mds.getColumnMetaData("STUDENT", "SVW_STUDENT_TIMETABLE_MASTER", "CHECKER_DATE_STAMP", session).getI_ColumnID();
             dbg("checkerDateColId"+checkerDateColId);       
             
             try{
                 
                 
               studentTimeTableMap=  readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"STUDENT"+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID,"STUDENT", "SVW_STUDENT_TIMETABLE_MASTER", session, dbSession, "Yes");
             ...
             if(tableExistence){
                ...
                Iterator<String>keyIterator= studentTimeTableMap.keySet().iterator();
                
                while(keyIterator.hasNext()){
                    
                    String key=keyIterator.next();
                    ArrayList<String>value=studentTimeTableMap.get(key).getRecord();
                    ...
                    if(bs.compareCheckerDate(checkerDateStamp,"TimeTable",instituteID,session,dbSession,inject)){
                        ...
                        dbts.deleteRecord("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"STUDENT"+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID,"STUDENT","SVW_STUDENT_TIMETABLE_MASTER", l_pkey, session);
        ...
        batchUtil.studentTimeTableArchProcessingSuccessHandler(instituteID, businessDate, studentID, inject, session, dbSession);
        }catch(DBValidationException ex){
          batchUtil.studentTimeTableArchProcessingErrorHandler(instituteID, businessDate, studentID, ex, inject, session, dbSession);
...
//         batchUtil.studentTimeTableArchProcessingErrorHandler(instituteID, businessDate, studentID, ex, inject, session, dbSession);
  ...
  private void detailTableProcessing(String instituteID,String studentID,String[] pkey)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      try{
          ...
          IMetaDataService mds=inject.getMetadataservice();
          String currentDate=bs.getCurrentDate();
          int tableId=mds.getTableMetaData("STUDENT", "SVW_STUDENT_TIMETABLE_DETAIL", session).getI_Tableid();
          
          
          Map<String,DBRecord>studentTimeTableDetailMap=  readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"STUDENT"+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID,"STUDENT", "SVW_STUDENT_TIMETABLE_DETAIL", session, dbSession);
          
          List<DBRecord> timeTableList=studentTimeTableDetailMap.values().stream().collect(Collectors.toList());
          ...
          for(int i=0;i<timeTableList.size();i++){
              ...
              String[] l_pkey={studentID,day,periodNo};
              
              dbts.deleteRecord("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"STUDENT"+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID,"STUDENT","SVW_STUDENT_TIMETABLE_DETAIL", l_pkey, session);
   ...
   private String[] getDetailRecordValues(ArrayList<String> recordList)throws BSProcessingException{
      ...
      try{
          ...
          String[] recordValues={recordList.get(0),recordList.get(1),recordList.get(2),recordList.get(3),recordList.get(4),recordList.get(5)};
