 ...
 */
@Stateless
public class CAttendanceArchBatchProcessing implements ICAttendanceArchBatchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public CAttendanceArchBatchProcessing() throws NamingException {
   ...
   public void processing (String instituteID,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside classAttendance batch processing");
    ...
    private void sequentialProcessing(String instituteID,String l_businessDate,String batchName)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      try{
           ...
           dbg("inside attendance  arch batch--->sequentialProcessing");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
           ICAttendanceArchProcessing classArch=inject.getClassAttendanceArchProcessing();
           batchUtil=inject.getBatchUtil(session);
           tc=inject.getTransactionControlService();
           
           Map<String,DBRecord>classAssArchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "CLASS_ATTENDANCE_ARCH_BATCH_STATUS", session, dbSession);
           ...
           for(int i=0;i<classList.size();i++){
              DBRecord classAttendanceRec=classList.get(i);
              String l_standard=classAttendanceRec.getRecord().get(1).trim();
              String l_section=classAttendanceRec.getRecord().get(2).trim();
              String l_status=classAttendanceRec.getRecord().get(4).trim();
               ...
               dbg("l_status"+l_status);
                ...
                if(l_status.equals("F")){
                    ...
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "CLASS_ATTENDANCE_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                    batchUtil.moveClassAttendanceArchRecordToHistory(instituteID,l_standard,l_section,l_businessDate,classAttendanceRec, session, dbSession, inject);
                    tc.commit(session, dbSession);
                }
      ...
      try{
           ...
           for(int i=startIndex;i<=endIndex;i++){
               ...
               if(i<classList.size()){
                ...
                DBRecord stdentAttendanceRec=classList.get(i);
                String l_standard=stdentAttendanceRec.getRecord().get(1).trim();
                String l_section=stdentAttendanceRec.getRecord().get(2).trim();
                String l_status=stdentAttendanceRec.getRecord().get(4).trim();
                ...
                dbg("l_status"+l_status);
                        ...
                        if(l_status.equals("F")){
                            ...
                            String[]l_pkey={instituteID,l_standard,l_section,l_businessDate};
                            dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "CLASS_ATTENDANCE_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                            batchUtil.moveClassAttendanceArchRecordToHistory(instituteID,l_standard,l_section,l_businessDate,stdentAttendanceRec, session, dbSession, inject);
                            tc.commit(session, dbSession);
                        }
     ...
     private void classIdentification(String instituteID,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
           ...
           int tableId=mds.getTableMetaData("BATCH", "CLASS_ATTENDANCE_ARCH_BATCH_STATUS",session).getI_Tableid();
           dbg("tableId"+tableId);
