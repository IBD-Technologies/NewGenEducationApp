 ...
 */
@Stateless
public class TTimeTableArchBatchProcessing implements ITTimeTableArchBatchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public TTimeTableArchBatchProcessing() throws NamingException {
   ...
   public void processing (String instituteID,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside teacherTimeTable batch processing");
    ...
    private void sequentialProcessing(String instituteID,String l_businessDate,String batchName)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      try{
           ...
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
           ITTimeTableArchProcessing teacherArch=inject.getTeacherTimeTableArchProcessing();
           batchUtil=inject.getBatchUtil(session);
           tc=inject.getTransactionControlService();
           
           Map<String,DBRecord>teacherAssArchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "TEACHER_TIME_TABLE_ARCH_BATCH_STATUS", session, dbSession);
           ...
           for(int i=0;i<teacherList.size();i++){
              DBRecord stdentTimeTableRec=teacherList.get(i);
              String l_teacherID=stdentTimeTableRec.getRecord().get(1).trim();
              String l_status=stdentTimeTableRec.getRecord().get(3).trim();
               ...
               dbg("l_status"+l_status);
                ...
                if(l_status.equals("F")){
                    ...
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "TEACHER_TIME_TABLE_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                    batchUtil.moveTeacherTimeTableArchRecordToHistory(instituteID,l_teacherID,l_businessDate,stdentTimeTableRec, session, dbSession, inject);
                    tc.commit(session, dbSession);
                }
     ...
     private void teacherIdentification(String instituteID,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
           ...
           int tableId=mds.getTableMetaData("BATCH", "TEACHER_TIME_TABLE_ARCH_BATCH_STATUS",session).getI_Tableid();
           dbg("tableId"+tableId);
