 ...
 */
@Stateless
public class TAttendanceArchBatchProcessing implements ITAttendanceArchBatchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public TAttendanceArchBatchProcessing() throws NamingException {
   ...
   public void processing (String instituteID,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside teacherAttendance batch processing");
        ...
        dbg("end of teacherAssignmenArch batch processing");
    ...
    private void sequentialProcessing(String instituteID,String l_businessDate,String batchName)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      try{
           ...
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
           ITAttendanceArchProcessing teacherArch=inject.getTeacherAttendanceArchProcessing();
           batchUtil=inject.getBatchUtil(session);
           tc=inject.getTransactionControlService();
           
           Map<String,DBRecord>teacherAssArchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "TEACHER_ATTENDANCE_ARCH_BATCH_STATUS", session, dbSession);
           ...
           for(int i=0;i<teacherList.size();i++){
              DBRecord stdentAttendanceRec=teacherList.get(i);
              String l_teacherID=stdentAttendanceRec.getRecord().get(1).trim();
              String l_status=stdentAttendanceRec.getRecord().get(3).trim();
               ...
               dbg("l_status"+l_status);
                ...
                if(l_status.equals("F")){
                    ...
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "TEACHER_ATTENDANCE_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                    batchUtil.moveTeacherAttendanceArchRecordToHistory(instituteID,l_teacherID,l_businessDate,stdentAttendanceRec, session, dbSession, inject);
                    tc.commit(session, dbSession);
                }
     ...
     private void parallelProcessing(String instituteID,String l_businessDate,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      ITransactionControlService tc=null;
      try{
           dbg("inside teacher assignment archival Processing -->parallel");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           ITAttendanceArchProcessing teacherArch=inject.getTeacherAttendanceArchProcessing();
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
     ...
     private void teacherIdentification(String instituteID,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
           ...
           int tableId=mds.getTableMetaData("BATCH", "TEACHER_ATTENDANCE_ARCH_BATCH_STATUS",session).getI_Tableid();
           dbg("tableId"+tableId);
