 ...
 */
@Stateless
public class TimeTableBatchProcessing implements ITimeTableBatchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public TimeTableBatchProcessing() throws NamingException {
   ...
   public void processing (String instituteID,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside timetable batch processing");
        ...
        dbg("end of time table batch processing");
        }catch(Exception ex){
           dbg(ex);
     ...
     private void sequentialProcessing(String instituteID,String l_businessDate,String batchName)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      try{
           ...
           dbg("inside time table batch--->sequentialProcessing");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IClassTimeTableProcessing classTimeTableProcessing=inject.getClassTimeTableProcessing();
           IDBTransactionService dbts=inject.getDBTransactionService();
           batchUtil=inject.getBatchUtil(session);
           tc=inject.getTransactionControlService();
           
           dbg("before reading time table batch table");
           Map<String,DBRecord>timeTableBatchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "TIMETABLE_BATCH_STATUS", session, dbSession);
           List<DBRecord>timeTableist=timeTableBatchMap.values().stream().filter(rec->rec.getRecord().get(4).trim().equals("F")||rec.getRecord().get(4).trim().equals("W")).collect(Collectors.toList());
           dbg("after reading time table batch table");
           dbg("timeTableist size "+timeTableist.size());
           
           for(int i=0;i<timeTableist.size();i++){
               DBRecord timeTableRecord=timeTableist.get(i);
               String l_standard=timeTableRecord.getRecord().get(1).trim();
               String l_section=timeTableRecord.getRecord().get(2).trim();
               String l_status=timeTableRecord.getRecord().get(4).trim();
               
               dbg("inside timeTableist iteration");
               ...
               dbg("l_status"+l_status);
                ...
                if(l_status.equals("F")){
                    ...
                    batchUtil.moveClassTimeTableRecordToHistory(instituteID,l_standard,l_section,l_businessDate,timeTableRecord, session, dbSession, inject);
                    tc.commit(session, dbSession);
                }
                
                classTimeTableProcessing.processing(instituteID,l_standard,l_section,l_businessDate,session);
           }
      ...
      ITransactionControlService tc=null;
      try{
           dbg("inside timeTable batch parallel Processing");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
           IClassTimeTableProcessing classTimeTableProcessing=inject.getClassTimeTableProcessing();
           ...
           dbg("before reading time table batch table");
           Map<String,DBRecord>timeTableBatchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "TIMETABLE_BATCH_STATUS", session, dbSession);
           List<DBRecord>timeTablelist=timeTableBatchMap.values().stream().filter(rec->rec.getRecord().get(4).trim().equals("F")||rec.getRecord().get(4).trim().equals("W")).collect(Collectors.toList());
           dbg("after reading time table batch table");
           dbg("timeTableist size "+timeTablelist.size());
//           Future<String>[] Result=new Future[no_of_threads];
           ...
           if(timeTablelist.size()%no_of_threads==0){
               
               no_of_execution=timeTablelist.size()/no_of_threads;
               
           }else{
               
               no_of_execution=(timeTablelist.size()/no_of_threads)+1;
           }
           ...
           for(int i=startIndex;i<=endIndex;i++){
               int j=0;
               if(i<timeTablelist.size()){
                   if(startIndex==i){
                       j=0;
                   }else{
                       j++;
                   }
               DBRecord timeTableRecord=timeTablelist.get(i);
               String l_standard=timeTableRecord.getRecord().get(1).trim();
               String l_section=timeTableRecord.getRecord().get(2).trim();
               String l_status=timeTableRecord.getRecord().get(4).trim();
               ...
               if(l_status.equals("F")){
                ...
                try{
                  ...
                  Result[j]= (Future<String>)classTimeTableProcessing.parallelProcessing(instituteID, l_standard, l_section,l_businessDate);
...
//                 if(threadCount==no_of_threads)
   ...
   private void classIdentification(String instituteID,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ITransactionControlService tc=null;
       try{
           dbg("inside timetable batch feeIdentification");
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           IDBTransactionService dbts=inject.getDBTransactionService();
           BusinessService bs=inject.getBusinessService(session);
           ...
           for(int i=0;i<classList.size();i++){
               ...
               DBRecord dbRec=readBuffer.readRecord("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"CLASS"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_standard+l_section+i_db_properties.getProperty("FOLDER_DELIMITER")+l_standard+l_section,"CLASS", "CLASS_TIMETABLE_MASTER", l_primaryKey, session, dbSession,true);
               
               String checkerDateStamp=dbRec.getRecord().get(5).trim();
               ...
               try{
                 ...
                 dbts.createRecord(session,"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", 130,instituteID,l_standard,l_section,l_businessDate,"W"," "," "," "," "); 
                 tc.commit(session, dbSession);
               }catch(DBValidationException ex){
