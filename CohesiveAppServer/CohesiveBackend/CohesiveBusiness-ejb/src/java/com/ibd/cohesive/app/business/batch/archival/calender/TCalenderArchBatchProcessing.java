 ...
 */
@Stateless
public class TCalenderArchBatchProcessing implements ITCalenderArchBatchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public TCalenderArchBatchProcessing() throws NamingException {
   ...
   public void processing (String instituteID,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside teacherCalender batch processing");
        ...
        if(recordExistence==false||l_eodStatus.equals("F")){
            ...
            teacherIdentification(instituteID,l_businessDate);
        ...
        dbg("end of teacherCalenderArch batch processing");
    ...
    private void sequentialProcessing(String instituteID,String l_businessDate,String batchName)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      try{
           ...
           dbg("inside calender arch batch--->sequentialProcessing");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
           ITCalenderArchProcessing teacherArch=inject.getTeacherCalenderArchProcessing();
           batchUtil=inject.getBatchUtil(session);
           tc=inject.getTransactionControlService();
           
           Map<String,DBRecord>teacherAssArchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "TEACHER_CALENDER_ARCH_BATCH_STATUS", session, dbSession);
           List<DBRecord>teacherList=teacherAssArchMap.values().stream().filter(rec->rec.getRecord().get(3).trim().equals("F")||rec.getRecord().get(3).trim().equals("W")).collect(Collectors.toList());
           
           for(int i=0;i<teacherList.size();i++){
              DBRecord teacherCalenderRec=teacherList.get(i);
              String l_teacherID=teacherCalenderRec.getRecord().get(1).trim();
              String l_status=teacherCalenderRec.getRecord().get(3).trim();
               
               dbg("inside teacherList iteration");
               dbg("l_status"+l_status);
                ...
                if(l_status.equals("F")){
                    ...
                    String[]l_pkey={instituteID,l_teacherID,l_businessDate};
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "TEACHER_CALENDER_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                    batchUtil.moveTeacherCalenderArchRecordToHistory(instituteID,l_teacherID,l_businessDate,teacherCalenderRec, session, dbSession, inject);
                    tc.commit(session, dbSession);
                }
                
                teacherArch.processing(l_teacherID, instituteID, l_businessDate, session);
           }
          dbg("end of calender batch--->sequentialProcessing");  
         batchUtil.batchSucessHandler(instituteID, l_businessDate, batchName, inject, session, dbSession);
        }catch(DBValidationException ex){
          batchUtil.batchErrorHandler(instituteID, l_businessDate, ex, batchName, inject, session, dbSession);
        }catch(BSValidationException ex){
     ...
     private void parallelProcessing(String instituteID,String l_businessDate,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      ITransactionControlService tc=null;
      try{
           dbg("inside teacher calender archival Processing -->parallel");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           ITCalenderArchProcessing teacherArch=inject.getTeacherCalenderArchProcessing();
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
           ...
           Map<String,DBRecord>teacherAssArchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "TEACHER_CALENDER_ARCH_BATCH_STATUS", session, dbSession);
           List<DBRecord>teacherList=teacherAssArchMap.values().stream().filter(rec->rec.getRecord().get(3).trim().equals("F")||rec.getRecord().get(3).trim().equals("W")).collect(Collectors.toList());
           dbg("teacherList"+teacherList.size());
           ...
           for(int i=startIndex;i<=endIndex;i++){
               ...
               if(i<teacherList.size()){
                ...
                try{
                  ...
                  Result[j]= (Future<String>)teacherArch.parallelProcessing(l_teacherID, instituteID, l_businessDate);
     ...
     private void teacherIdentification(String instituteID,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
           ...
           ArrayList<String>teacherList=bs.getTeachersOfTheInstitute(instituteID, session, dbSession, inject);
           dbg("l_teacherOfTheGroup"+teacherList.size());        
           IMetaDataService mds=inject.getMetadataservice();
           int tableId=mds.getTableMetaData("BATCH", "TEACHER_CALENDER_ARCH_BATCH_STATUS",session).getI_Tableid();
           dbg("tableId"+tableId);
           
                for(int j=0;j<teacherList.size();j++){

                     String l_teacherID=teacherList.get(j).trim();
                     dbg("l_teacherID"+l_teacherID);
                   
                    try{ 
                     
                     dbts.createRecord(session,"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", tableId,instituteID,l_teacherID,l_businessDate,"W"," "," ");   
                      tc.commit(session, dbSession);
                     }catch(DBValidationException ex){
