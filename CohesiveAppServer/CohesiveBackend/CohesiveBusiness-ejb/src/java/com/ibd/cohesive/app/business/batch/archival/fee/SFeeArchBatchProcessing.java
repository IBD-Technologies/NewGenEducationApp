 ...
 */
@Stateless
public class SFeeArchBatchProcessing implements ISFeeArchBatchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public SFeeArchBatchProcessing() throws NamingException {
   ...
   public void processing (String instituteID,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside studentFee batch processing");
    ...
    private void sequentialProcessing(String instituteID,String l_businessDate,String batchName)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      try{
           ...
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
           ISFeeArchProcessing studentArch=inject.getStudentFeeArchProcessing();
           batchUtil=inject.getBatchUtil(session);
           tc=inject.getTransactionControlService();
           
           Map<String,DBRecord>studentAssArchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "STUDENT_FEE_ARCH_BATCH_STATUS", session, dbSession);
           ...
           for(int i=0;i<studentList.size();i++){
              DBRecord stdentFeeRec=studentList.get(i);
              String l_studentID=stdentFeeRec.getRecord().get(1).trim();
              String l_status=stdentFeeRec.getRecord().get(3).trim();
               ...
               dbg("l_status"+l_status);
                ...
                if(l_status.equals("F")){
                    ...
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "STUDENT_FEE_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                    batchUtil.moveStudentFeeArchRecordToHistory(instituteID,l_studentID,l_businessDate,stdentFeeRec, session, dbSession, inject);
                    tc.commit(session, dbSession);
                }
     ...
     private void studentIdentification(String instituteID,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
           ...
           int tableId=mds.getTableMetaData("BATCH", "STUDENT_FEE_ARCH_BATCH_STATUS",session).getI_Tableid();
           dbg("tableId"+tableId);
