 ...
 */
package com.ibd.cohesive.app.business.batch.archival.notification;
 ...
 */
@Stateless
public class InsNotificationArchBatchProcessing implements IInsNotificationArchBatchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public InsNotificationArchBatchProcessing() throws NamingException {
   ...
   public void processing (String instituteID,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside instituteNotification batch processing");
        ...
        if(recordExistence==false||l_eodStatus.equals("F")){
            ...
            DBRecord insRec=readBuffer.readRecord("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "INSTITUTE_NOTIFICATION_ARCH_BATCH_STATUS", l_pkey, session, dbSession);
            String l_status=insRec.getRecord().get(2).trim();
                       ...
                       if(l_status.equals("F")){
                            ...
                            dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "INSTITUTE_NOTIFICATION_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                            batchUtil.moveInstituteNotificationArchRecordToHistory(instituteID,l_businessDate,insRec, session, dbSession, inject);
                            tc.commit(session, dbSession);
                         }
     ...
     private void instituteProcessing(String instituteID,String businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
        ...
        try{
             ...
             int tableId=mds.getTableMetaData("INSTITUTE", "IVW_NOTIFICATION_MASTER", session).getI_Tableid();
             Map<String,DBRecord> instituteNotificationMap=null;
                    ...
                    column_to_Update=new HashMap();
                    column_to_Update.put("4", startTime);
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate, "BATCH", "INSTITUTE_NOTIFICATION_ARCH_BATCH_STATUS", l_primaryKey, column_to_Update,session); 
                    tc.commit(session, dbSession);
             
             
             
             boolean tableExistence=true;
             int checkerDateColId=mds.getColumnMetaData("INSTITUTE", "IVW_NOTIFICATION_MASTER", "CHECKER_DATE_STAMP", session).getI_ColumnID();
             dbg("checkerDateColId"+checkerDateColId);       
             
             try{
                 
                 
               instituteNotificationMap=  readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID,"INSTITUTE", "IVW_NOTIFICATION_MASTER", session, dbSession, "Yes");
             ...
             if(tableExistence){
                ...
                Iterator<String>keyIterator= instituteNotificationMap.keySet().iterator();
                
                while(keyIterator.hasNext()){
                    
                    String key=keyIterator.next();
                    ArrayList<String>value=instituteNotificationMap.get(key).getRecord();
                    String checkerDateStamp=value.get(checkerDateColId-1).trim();
                    dbg("key"+key);
                    dbg("checkerDateStamp"+checkerDateStamp);
                    
                    if(bs.compareCheckerDate(checkerDateStamp,"Notification",instituteID,session,dbSession,inject)){
                        
                        String[] l_pkey=key.split("~");
                        
                        dbg("before delete record call for key"+key);
                        
                        dbts.deleteRecord("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID,"INSTITUTE", "IVW_NOTIFICATION_MASTER", l_pkey, session);
        ...
        batchUtil.instituteNotificationArchProcessingSuccessHandler(instituteID, businessDate, inject, session, dbSession);
        }catch(DBValidationException ex){
          batchUtil.instituteNotificationArchProcessingErrorHandler(instituteID, businessDate, ex, inject, session, dbSession);
//        }catch(BSValidationException ex){
//         batchUtil.studentNotificationArchProcessingErrorHandler(instituteID, businessDate, studentID, ex, inject, session, dbSession);
        }catch(DBProcessingException ex){
          dbg(ex);
          batchUtil.instituteNotificationArchProcessingErrorHandler(instituteID, businessDate, ex, inject, session, dbSession);
        }catch(Exception ex){
     ...
     private void addRecordForInstituteStatusTable(String instituteID,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
           ...
           ITransactionControlService tc=inject.getTransactionControlService();
           IMetaDataService mds=inject.getMetadataservice();
           int tableId=mds.getTableMetaData("BATCH", "INSTITUTE_NOTIFICATION_ARCH_BATCH_STATUS",session).getI_Tableid();
           dbg("tableId"+tableId);
