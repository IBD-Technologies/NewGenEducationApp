 ...
 */
@Stateless
public class CExamArchProcessing implements ICExamArchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public CExamArchProcessing() throws NamingException {
  ...
  public void processing (String standard,String section,String instituteID,String businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
             ...
             dbg("inside class exam arch batch processing");
             String currentDate=bs.getCurrentDate();
             int tableId=mds.getTableMetaData("CLASS", "CLASS_EXAM_SCHEDULE_MASTER", session).getI_Tableid();
             Map<String,DBRecord> classExamMap=null;
             boolean tableExistence=true;
                    ...
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate, "BATCH", "CLASS_EXAM_ARCH_BATCH_STATUS", l_primaryKey, column_to_Update,session); 
                    tc.commit(session, dbSession);
             
             
             
             int checkerDateColId=mds.getColumnMetaData("CLASS", "CLASS_EXAM_SCHEDULE_MASTER", "CHECKER_DATE_STAMP", session).getI_ColumnID();
             dbg("checkerDateColId"+checkerDateColId);       
             
             try{
                 
                 
               classExamMap=  readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"CLASS"+i_db_properties.getProperty("FOLDER_DELIMITER")+standard+section+i_db_properties.getProperty("FOLDER_DELIMITER")+standard+section,"CLASS", "CLASS_EXAM_SCHEDULE_MASTER", session, dbSession, "Yes");
             ...
             if(tableExistence){
                ...
                Iterator<String>keyIterator= classExamMap.keySet().iterator();
                
                while(keyIterator.hasNext()){
                    
                    String key=keyIterator.next();
                    ArrayList<String>value=classExamMap.get(key).getRecord();
                    ...
                    if(bs.compareCheckerDate(checkerDateStamp,"Exam",instituteID,session,dbSession,inject)){
                        ...
                        dbts.deleteRecord("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"CLASS"+i_db_properties.getProperty("FOLDER_DELIMITER")+standard+section+i_db_properties.getProperty("FOLDER_DELIMITER")+standard+section,"CLASS","CLASS_EXAM_SCHEDULE_MASTER", l_pkey, session);
        ...
        batchUtil.classExamArchProcessingSuccessHandler(instituteID, businessDate, standard,section, inject, session, dbSession);
        }catch(DBValidationException ex){
          batchUtil.classExamArchProcessingErrorHandler(instituteID, businessDate, standard,section, ex, inject, session, dbSession);
//        }catch(BSValidationException ex){
//         batchUtil.classExamArchProcessingErrorHandler(instituteID, businessDate, classID, ex, inject, session, dbSession);
  ...
  private String[] getRecordValues(ArrayList<String> recordList)throws BSProcessingException{
      ...
      try{
          ...
          String[] recordValues={recordList.get(0),recordList.get(1),recordList.get(2),recordList.get(3),recordList.get(4),recordList.get(5),recordList.get(6),recordList.get(7),recordList.get(8),recordList.get(9),recordList.get(10),recordList.get(11)};
  ...
  private void detailTableProcessing(String instituteID,String standard,String section,String[] pkey)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      try{
          ...
          String currentDate=bs.getCurrentDate();
          int tableId=mds.getTableMetaData("CLASS", "CLASS_EXAM_SCHEDULE_DETAIL", session).getI_Tableid();
          dbg("l_exam"+l_exam);
          dbg("tableId"+tableId);
          
          
          Map<String,DBRecord>classExamDetailMap=  readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"CLASS"+i_db_properties.getProperty("FOLDER_DELIMITER")+standard+section+i_db_properties.getProperty("FOLDER_DELIMITER")+standard+section,"CLASS", "CLASS_EXAM_SCHEDULE_DETAIL", session, dbSession);
          
          List<DBRecord> examWiseList=classExamDetailMap.values().stream().filter(rec->rec.getRecord().get(2).trim().equals(l_exam)).collect(Collectors.toList());
          
          for(int i=0;i<examWiseList.size();i++){
              
              ArrayList<String>examList=examWiseList.get(i).getRecord();
              String l_subjectID=examList.get(3).trim();
              dbg("l_subjectID"+l_subjectID);
              String[] l_pkey={standard,section,l_exam,l_subjectID};
              
              dbts.deleteRecord("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"CLASS"+i_db_properties.getProperty("FOLDER_DELIMITER")+standard+section+i_db_properties.getProperty("FOLDER_DELIMITER")+standard+section,"CLASS","CLASS_EXAM_SCHEDULE_DETAIL", l_pkey, session);
