 ...
 */
@Stateless
public class CExamArchBatchProcessing implements ICExamArchBatchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public CExamArchBatchProcessing() throws NamingException {
   ...
   public void processing (String instituteID,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside classExam batch processing");
    ...
    private void sequentialProcessing(String instituteID,String l_businessDate,String batchName)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      ...
      try{
           ...
           IBDProperties i_db_properties=session.getCohesiveproperties();
           IDBTransactionService dbts=inject.getDBTransactionService();
           ICExamArchProcessing classArch=inject.getClassExamArchProcessing();
           batchUtil=inject.getBatchUtil(session);
           tc=inject.getTransactionControlService();
           
           Map<String,DBRecord>classAssArchMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "CLASS_EXAM_ARCH_BATCH_STATUS", session, dbSession);
           ...
           for(int i=0;i<classList.size();i++){
              DBRecord classExamRec=classList.get(i);
              String l_standard=classExamRec.getRecord().get(1).trim();
              String l_section=classExamRec.getRecord().get(2).trim();
              String l_status=classExamRec.getRecord().get(4).trim();
               ...
               dbg("l_status"+l_status);
                ...
                if(l_status.equals("F")){
                    ...
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "CLASS_EXAM_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                    batchUtil.moveClassExamArchRecordToHistory(instituteID,l_standard,l_section,l_businessDate,classExamRec, session, dbSession, inject);
                    tc.commit(session, dbSession);
                }
      ...
      try{
           ...
           for(int i=startIndex;i<=endIndex;i++){
               ...
               if(i<classList.size()){
                ...
                DBRecord stdentExamRec=classList.get(i);
                String l_standard=stdentExamRec.getRecord().get(1).trim();
                String l_section=stdentExamRec.getRecord().get(2).trim();
                String l_status=stdentExamRec.getRecord().get(4).trim();
                ...
                dbg("l_status"+l_status);
                        ...
                        if(l_status.equals("F")){
                            ...
                            String[]l_pkey={instituteID,l_standard,l_section,l_businessDate};
                            dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "CLASS_EXAM_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                            batchUtil.moveClassExamArchRecordToHistory(instituteID,l_standard,l_section,l_businessDate,stdentExamRec, session, dbSession, inject);
                            tc.commit(session, dbSession);
                        }
     ...
     private void classIdentification(String instituteID,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
           ...
           int tableId=mds.getTableMetaData("BATCH", "CLASS_EXAM_ARCH_BATCH_STATUS",session).getI_Tableid();
           dbg("tableId"+tableId);
