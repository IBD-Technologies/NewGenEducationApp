 ...
 */
package com.ibd.cohesive.app.business.batch.archival.payment;
 ...
 */
@Stateless
public class InsPaymentArchBatchProcessing implements IInsPaymentArchBatchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public InsPaymentArchBatchProcessing() throws NamingException {
   ...
   public void processing (String instituteID,String batchName,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside institutePayment batch processing");
        ...
        if(recordExistence==false||l_eodStatus.equals("F")){
            ...
            DBRecord insRec=readBuffer.readRecord("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "INSTITUTE_PAYMENT_ARCH_BATCH_STATUS", l_pkey, session, dbSession);
            String l_status=insRec.getRecord().get(2).trim();
                       ...
                       if(l_status.equals("F")){
                            ...
                            dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "INSTITUTE_PAYMENT_ARCH_BATCH_STATUS", l_pkey, column_to_Update,session); 
                            batchUtil.moveInstitutePaymentArchRecordToHistory(instituteID,l_businessDate,insRec, session, dbSession, inject);
                            tc.commit(session, dbSession);
                         }
     ...
     private void instituteProcessing(String instituteID,String businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
        ...
        try{
             ...
             int tableId=mds.getTableMetaData("INSTITUTE", "IVW_INSTITUTE_PAYMENT", session).getI_Tableid();
             Map<String,DBRecord> institutePaymentMap=null;
                    String startTime=bs.getCurrentDateTime();
                    String[] l_primaryKey={instituteID,businessDate};
                    Map<String,String>column_to_Update=new HashMap();
                    column_to_Update=new HashMap();
                    column_to_Update.put("4", startTime);
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate, "BATCH", "INSTITUTE_PAYMENT_ARCH_BATCH_STATUS", l_primaryKey, column_to_Update,session); 
                    tc.commit(session, dbSession);
             
             
             
             boolean tableExistence=true;
             int checkerDateColId=mds.getColumnMetaData("INSTITUTE", "IVW_INSTITUTE_PAYMENT", "CHECKER_DATE_STAMP", session).getI_ColumnID();
             dbg("checkerDateColId"+checkerDateColId);       
             
             try{
                 
                 
               institutePaymentMap=  readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID,"INSTITUTE", "IVW_INSTITUTE_PAYMENT", session, dbSession, "Yes");
             ...
             if(tableExistence){
                ...
                Iterator<String>keyIterator= institutePaymentMap.keySet().iterator();
                
                while(keyIterator.hasNext()){
                    
                    String key=keyIterator.next();
                    ArrayList<String>value=institutePaymentMap.get(key).getRecord();
                    String checkerDateStamp=value.get(checkerDateColId-1).trim();
                    dbg("key"+key);
                    dbg("checkerDateStamp"+checkerDateStamp);
                    
                    if(bs.compareCheckerDate(checkerDateStamp,"Payment",instituteID,session,dbSession,inject)){
                        
                        String[] l_pkey=key.split("~");
                        
                        dbg("before delete record call for key"+key);
                        
                        dbts.deleteRecord("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID,"INSTITUTE", "IVW_INSTITUTE_PAYMENT", l_pkey, session);
        ...
        batchUtil.institutePaymentArchProcessingSuccessHandler(instituteID, businessDate, inject, session, dbSession);
        }catch(DBValidationException ex){
          batchUtil.institutePaymentArchProcessingErrorHandler(instituteID, businessDate, ex, inject, session, dbSession);
//        }catch(BSValidationException ex){
//         batchUtil.studentPaymentArchProcessingErrorHandler(instituteID, businessDate, studentID, ex, inject, session, dbSession);
        }catch(DBProcessingException ex){
          dbg(ex);
          batchUtil.institutePaymentArchProcessingErrorHandler(instituteID, businessDate, ex, inject, session, dbSession);
        }catch(Exception ex){
     ...
     private void addRecordForInstituteStatusTable(String instituteID,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
           ...
           ITransactionControlService tc=inject.getTransactionControlService();
           IMetaDataService mds=inject.getMetadataservice();
           int tableId=mds.getTableMetaData("BATCH", "INSTITUTE_PAYMENT_ARCH_BATCH_STATUS",session).getI_Tableid();
           dbg("tableId"+tableId);
