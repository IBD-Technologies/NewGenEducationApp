 ...
 */
@Stateless
public class AssignmentProcessing implements IAssignmentProcessing {
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public AssignmentProcessing() throws NamingException {
        inject = new AppDependencyInjection();
        session = new CohesiveSession();
        dbSession = new DBSession(session);
    }
      public void processing (String instituteID,String assignmentID,String groupID,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
        ...
        dbg("inside assignment processing");
        

        Map<String,String>column_to_Update=new HashMap();
        column_to_Update.put("9", startTime);
        String[] l_pkeyy={instituteID,assignmentID,l_businessDate};
        ...
        tc.commit(session, dbSession);
              ...
              studentIdentification(instituteID,l_businessDate,assignmentID,groupID) ;   
              
              String[] l_pkey={"Assignment"};
            ...
            if(no_of_threads==0){
                ...
                sequentialProcessing(instituteID,assignmentID,l_businessDate);
            }else{

                parallelProcessing(instituteID,assignmentID,l_businessDate,no_of_threads);
            }
                    
                    
         batchUtil.assignmentProcessingSuccessHandler(instituteID, l_businessDate, assignmentID, inject, session, dbSession);
        dbg("end of assignment processing");
        }catch(DBValidationException ex){
//          tc.rollBack(session, dbSession);
          batchUtil.assignmentProcessingErrorHandler(instituteID, l_businessDate, assignmentID, ex, inject, session, dbSession);
...
//           tc.rollBack(session, dbSession);
    ...
    public void processing(String instituteID,String assignmentID,String groupID,String l_businessDate,CohesiveSession session) throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
           ...
           processing(instituteID,assignmentID,groupID,l_businessDate);
     ...
     private void sequentialProcessing(String instituteID,String assignmentID,String l_businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      BatchUtil batchUtil=null;   
      ITransactionControlService tc=null;
      try{
           dbg("inside assignmentProcessing -->sequential");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           IBDProperties i_db_properties=session.getCohesiveproperties();
           tc=inject.getTransactionControlService();
           IStudentAssignmentProcessing studentOtherActProcessing=inject.getStudentAssignmentProcessing();
           IDBTransactionService dbts=inject.getDBTransactionService();
           batchUtil=inject.getBatchUtil(session);
      
            Map<String,DBRecord>studentEodMap=readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+"Assignment"+i_db_properties.getProperty("FOLDER_DELIMITER")+assignmentID, "BATCH", "STUDENT_ASSIGNMENT_EOD_STATUS", session, dbSession);
...
//               for(int i=0;i<filteredStudentList.size();i++){
              while(studentIterator.hasNext()){
                   ...
                   DBRecord assignmentRecord=studentIterator.next();
                   String l_studentID=assignmentRecord.getRecord().get(2).trim();
                   String l_status=assignmentRecord.getRecord().get(4).trim();
                   dbg("l_studentID"+l_studentID);
                   dbg("l_status"+l_status);

                   if(l_status.equals("F")){

                        int assignmentHistoryMax=batchUtil.getStudentAssignmentHistoryMaxSequence(l_businessDate, instituteID, assignmentID, l_studentID, inject, session, dbSession);

                        if(assignmentHistoryMax<=5){

                            studentList.add(assignmentRecord);
                        }
                    ...
                    for(int j=0;j<studentList.size();j++){
                         DBRecord studentAssignmentRecord=studentList.get(j);
...
//                             batchUtil.getStudentAssignmentHistoryMaxSequence(l_businessDate, instituteID, assignmentID, l_studentID, inject, session, dbSession);
                             
                             
                            Map<String,String>column_to_Update=new HashMap();
                            column_to_Update=new HashMap();
                            column_to_Update.put("5", "W");
                            String[]l_pkey={instituteID,assignmentID,l_studentID,l_businessDate};
                            ...
                            batchUtil.moveStudentAssignmentRecordToHistory(instituteID,assignmentID,l_studentID,l_businessDate,studentAssignmentRecord, session, dbSession, inject);
                         }
                         tc.commit(session, dbSession);
                         
                         dbg("before studentAssProcessing call");
                         studentOtherActProcessing.processing(instituteID,l_studentID,assignmentID,l_businessDate,session);
         ...
         batchUtil.assignmentProcessingSuccessHandler(instituteID, l_businessDate, assignmentID, inject, session, dbSession);
        dbg("end of assignmentProcessing -->sequential");
        }catch(DBValidationException ex){
//          tc.rollBack(session, dbSession);
          batchUtil.assignmentProcessingErrorHandler(instituteID, l_businessDate, assignmentID, ex, inject, session, dbSession);
        }catch(DBProcessingException ex){
...
//           tc.rollBack(session, dbSession);
    ...
    private void parallelProcessing(String instituteID,String assignmentID,String l_businessDate,int no_of_threads)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
      BatchUtil batchUtil=null; 
      ITransactionControlService tc=null;
      try{
           dbg("inside assignmentProcessing -->parallel");
           IDBReadBufferService readBuffer=inject.getDBReadBufferService();
           IBDProperties i_db_properties=session.getCohesiveproperties();
            IStudentAssignmentProcessing studentOtherActProcessing=inject.getStudentAssignmentProcessing();
           IDBTransactionService dbts=inject.getDBTransactionService();
...
//               for(int i=0;i<filteredStudentList.size();i++){
           ...
           for(int i=startIndex;i<=endIndex;i++){
               ...
               if(i<studentList.size()){
                ...
                DBRecord studentAssignmentRecord=studentList.get(i);
                ...
                try{
                  ...
                  Result[j]= (Future<String>)studentOtherActProcessing.parallelProcessing(instituteID,l_studentID, assignmentID, l_businessDate);
          ...
          dbg("end of assignmentProcessing -->parellel");
        }catch(DBValidationException ex){
//          tc.rollBack(session, dbSession);
          batchUtil.assignmentProcessingErrorHandler(instituteID, l_businessDate, assignmentID, ex, inject, session, dbSession);
        }catch(DBProcessingException ex){
...
//           tc.rollBack(session, dbSession);
    ...
    @Asynchronous
   public Future<String> parallelProcessing(String instituteID,String assignmentID,String groupID,String l_businessDate) throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
 ...
 private void studentIdentification(String instituteID,String l_businessDate,String assignmentID,String p_groupID)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
           ...
           dbg("l_studentOfTheGroup"+l_studentOfTheGroup.size());        
                ...
                for(int j=0;j<l_studentOfTheGroup.size();j++){
                    ...
                    try{ 
                         ...
                         dbts.createRecord(session,"INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+"Assignment"+i_db_properties.getProperty("FOLDER_DELIMITER")+assignmentID, "BATCH", 100,instituteID,assignmentID,l_studentID,l_businessDate,"W"," "," "," ");   
                          tc.commit(session, dbSession);
                     }catch(DBValidationException ex){
                        tc.rollBack(session, dbSession);
...
//        column_to_Update.put("9", startTime);
//        String[] l_pkeyy={instituteID,assignmentID,l_businessDate};
//        dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "ASSIGNMENT_EOD_STATUS", l_pkeyy, column_to_Update,session); 
//        tc.commit(session, dbSession);
//                
//              studentIdentification(instituteID,l_businessDate,assignmentID,groupID) ;   
//              
...
//           IBDProperties i_db_properties=session.getCohesiveproperties();
//           tc=inject.getTransactionControlService();
//           IStudentAssignmentProcessing studentAssignmentProcessing=inject.getStudentAssignmentProcessing();
//           IDBTransactionService dbts=inject.getDBTransactionService();
//           batchUtil=inject.getBatchUtil(session);
//      
//            Map<String,DBRecord>studentEodMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", "STUDENT_ASSIGNMENT_EOD_STATUS", session, dbSession);
...
//                         
//                         dbg("before studentAssProcessing call");
//                         studentAssignmentProcessing.processing(instituteID,l_studentID,assignmentID,l_businessDate,session);
//                         dbg("after studentAssProcessing call");
//                         
//                   }
//    
...
//                try{
//                    
//                  Result[j]= (Future<String>)studentAssignmentProcessing.parallelProcessing(instituteID,l_studentID, assignmentID, l_businessDate);
//               
//                } catch (EJBException ex) {
//                   dispatchFailed=true;
//                }
...
//                    try{ 
//                     
//                     dbts.createRecord(session,"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+l_businessDate, "BATCH", 100,instituteID,p_asiignmentID,l_studentID,l_businessDate,"W"," "," "," ");   
//                      tc.commit(session, dbSession);
//                     }catch(DBValidationException ex){
//                   
//                       if(!ex.toString().contains("DB_VAL_009")){
