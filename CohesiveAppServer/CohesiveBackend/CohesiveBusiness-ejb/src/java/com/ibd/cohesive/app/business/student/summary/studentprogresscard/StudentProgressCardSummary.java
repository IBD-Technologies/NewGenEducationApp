 ...
 * @author DELL
 */
//@Local(IStudentProgressCardSummary.class)
@Remote(IStudentProgressCardSummary.class)
@Stateless
public class StudentProgressCardSummary implements IStudentProgressCardSummary{
    ...
    public StudentProgressCardSummary(){
        try {
    ...
    {     
     ...
     public JsonObject processing(JsonObject requestJson)throws BSProcessingException,BSValidationException,DBValidationException,DBProcessingException{
       ...
       dbg("inside StudentProgressCardSummary--->processing");
       dbg("StudentProgressCardSummary--->Processing--->I/P--->requestJson"+requestJson.toString());
       ...
       BusinessEJB<IStudentProgressCardSummary>studentProgressCardEJB=new BusinessEJB();
 ...
 return response;
    ...
    private  void buildBOfromReq(JsonObject p_request)throws BSProcessingException,DBProcessingException{
      StudentProgressCardBO studentProgressCard=new StudentProgressCardBO();
      RequestBody<StudentProgressCardBO> reqBody = new RequestBody<StudentProgressCardBO>(); 
      ...
      BSValidation bsv=inject.getBsv(session);
      studentProgressCard.filter=new StudentProgressCardFilter();
      studentProgressCard.filter.setStudentID(l_filterObject.getString("studentID"));
      studentProgressCard.filter.setStudentName(l_filterObject.getString("studentName"));
//      studentProgressCard.filter.setRecordStatus(l_filterObject.getString("recordStat"));
//      studentProgressCard.filter.setAuthStatus(l_filterObject.getString("authStat"));
//      studentProgressCard.filter.setExam(l_filterObject.getString("exam"));
      ...
      if(l_filterObject.getString("class").equals("Select option")){
          ...
          studentProgressCard.filter.setStandard(l_class.split("/")[0]);
          studentProgressCard.filter.setSection(l_class.split("/")[1]);
      ...
      if(l_filterObject.getString("signStatus").equals("Select option")){
          ...
          studentProgressCard.filter.setSignStatus(l_filterObject.getString("signStatus"));
      }
    ...
    public void view()throws DBValidationException,DBProcessingException,BSProcessingException,BSValidationException{
        ...
        RequestBody<StudentProgressCardBO> reqBody = request.getReqBody();
        String l_instituteID=request.getReqHeader().getInstituteID();
        IDBReadBufferService readBuffer= inject.getDBReadBufferService();
        IPDataService pds=inject.getPdataservice();
        StudentProgressCardBO studentProgressCard=reqBody.get();
        String l_studentID=studentProgressCard.getFilter().getStudentID();
        String l_standard=studentProgressCard.getFilter().getStandard();
        String l_section=studentProgressCard.getFilter().getSection();
        String l_exam=studentProgressCard.getFilter().getExam();
        String l_signStatus=studentProgressCard.getFilter().getSignStatus();
        ...
        if(l_signStatus.equals("Y")){    
            ...
            try{
                ...
                Map<String,DBRecord>markMap=readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"CLASS"+i_db_properties.getProperty("FOLDER_DELIMITER")+fileName+i_db_properties.getProperty("FOLDER_DELIMITER")+l_exam,"CLASS","STUDENT_PROGRESS_CARD_SIGNATURE", session, dbSession);
...
//            try{
//            
//                Map<String,DBRecord>markMap=readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"CLASS"+i_db_properties.getProperty("FOLDER_DELIMITER")+fileName+i_db_properties.getProperty("FOLDER_DELIMITER")+l_exam,"CLASS","STUDENT_MARKS", session, dbSession);
//
             ...
             try{
                ...
                while(keyIterator.hasNext()){
                    ...
                    try{
                       ...
                       readBuffer.readRecord("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+l_instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"CLASS"+i_db_properties.getProperty("FOLDER_DELIMITER")+fileName+i_db_properties.getProperty("FOLDER_DELIMITER")+l_exam,"CLASS","STUDENT_PROGRESS_CARD_SIGNATURE", l_pkey, session, dbSession);
...
//           dbg("l_exam"+l_exam);
           ...
           studentProgressCard.result=new StudentProgressCardResult[l_progreassList.size()];
           for(int i=0;i<l_progreassList.size();i++){
               
               studentProgressCard.result[i]=new StudentProgressCardResult();
               
               studentProgressCard.result[i].setStudentID(l_progreassList.get(i).getStudentID());
               studentProgressCard.result[i].setStudentName(bs.getStudentName(l_progreassList.get(i).getStudentID(), instituteID, session, dbSession, inject));
               studentProgressCard.result[i].setSignStatus(l_progreassList.get(i).getSignStatus());
               studentProgressCard.result[i].setExam(l_exam);
               
          }    
           
           if(studentProgressCard.result==null||studentProgressCard.result.length==0){
               session.getErrorhandler().log_app_error("BS_VAL_013", null);
               throw new BSValidationException("BSValidationException");
           }
    ...
    public JsonObject buildJsonResFromBO()throws BSProcessingException{
        ...
        for(int i=0;i<studentProgressCard.result.length;i++){
            
            resultArray.add(Json.createObjectBuilder().add("studentID", studentProgressCard.result[i].getStudentID())
                                                      .add("studentName", studentProgressCard.result[i].getStudentName())
                                                      .add("signStatus", studentProgressCard.result[i].getSignStatus())
                                                      .add("exam", studentProgressCard.result[i].getExam())
                                                      );
        }

           filter=Json.createObjectBuilder()  .add("studentID",studentProgressCard.filter.getStudentID())
                                              .add("studentName",studentProgressCard.filter.getStudentName())
                                              .add("signStatus", studentProgressCard.filter.getSignStatus())
                                              .add("exam", studentProgressCard.filter.getExam())
                                              .add("class",studentProgressCard.filter.getStandard()+"/"+studentProgressCard.filter.getSection())
                                              .build();
    ...
    private boolean filterMandatoryValidation(ErrorHandler errhandler)throws BSProcessingException,BSValidationException{
      boolean status=true;
        ...
        RequestBody<StudentProgressCardBO> reqBody = request.getReqBody();
        StudentProgressCardBO studentProgressCard =reqBody.get();
        String standard=studentProgressCard.getFilter().getStandard();
        String section=studentProgressCard.getFilter().getSection();
        String studentID=studentProgressCard.getFilter().getStudentID();
        BusinessService bs=inject.getBusinessService(session);
        String userID=request.getReqHeader().getUserID();
        String userType=bs.getUserType(userID, session, dbSession, inject);
        String studentName=studentProgressCard.getFilter().getStudentName();
        String instituteID=request.getReqHeader().getInstituteID();
        studentID=bs.studentValidation(studentID, studentName, instituteID, session, dbSession, inject);

        studentProgressCard.getFilter().setStudentID(studentID);
       ...
       if(userType.equals("P")){
            ...
            if(studentProgressCard.getFilter().getExam()==null||studentProgressCard.getFilter().getExam().isEmpty()){
