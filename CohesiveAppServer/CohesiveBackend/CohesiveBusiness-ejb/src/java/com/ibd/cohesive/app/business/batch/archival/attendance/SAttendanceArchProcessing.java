 ...
 */
@Stateless
public class SAttendanceArchProcessing implements ISAttendanceArchProcessing{
    AppDependencyInjection inject;
    CohesiveSession session;
    DBSession dbSession;
    
     public SAttendanceArchProcessing() throws NamingException {
  ...
  public void processing (String studentID,String instituteID,String businessDate)throws DBProcessingException,DBValidationException,BSProcessingException,BSValidationException{
       ...
       try{
             ...
             int tableId=mds.getTableMetaData("STUDENT", "SVW_STUDENT_ATTENDANCE", session).getI_Tableid();
             Map<String,DBRecord> studentAttendanceMap=null;
             boolean tableExistence=true;
                    ...
                    dbts.updateColumn("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+businessDate, "BATCH", "STUDENT_ATTENDANCE_ARCH_BATCH_STATUS", l_primaryKey, column_to_Update,session); 
                    tc.commit(session, dbSession);
             
             
             
             int yearColId=mds.getColumnMetaData("STUDENT", "SVW_STUDENT_ATTENDANCE", "YEAR", session).getI_ColumnID();
             dbg("yearColId"+yearColId);       
             
             try{
                 
                 
               studentAttendanceMap=  readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"STUDENT"+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID,"STUDENT", "SVW_STUDENT_ATTENDANCE", session, dbSession, "Yes");
             ...
             if(tableExistence){
                ...
                Iterator<String>keyIterator= studentAttendanceMap.keySet().iterator();
                
                while(keyIterator.hasNext()){
                    
                    String key=keyIterator.next();
                    ArrayList<String>value=studentAttendanceMap.get(key).getRecord();
                    ...
                    if(bs.compareYear(year,"Attendance",instituteID,session,dbSession,inject)){
                        ...
                        dbts.deleteRecord("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"STUDENT"+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID+i_db_properties.getProperty("FOLDER_DELIMITER")+studentID,"STUDENT","SVW_STUDENT_ATTENDANCE", l_pkey, session);
        ...
        batchUtil.studentAttendanceArchProcessingSuccessHandler(instituteID, businessDate, studentID, inject, session, dbSession);
        }catch(DBValidationException ex){
          batchUtil.studentAttendanceArchProcessingErrorHandler(instituteID, businessDate, studentID, ex, inject, session, dbSession);
