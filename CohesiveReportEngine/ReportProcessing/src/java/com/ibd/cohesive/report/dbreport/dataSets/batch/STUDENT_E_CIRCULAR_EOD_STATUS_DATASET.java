 ...
 * @author ibdtech
 */
public class STUDENT_E_CIRCULAR_EOD_STATUS_DATASET {
    public ArrayList<STUDENT_E_CIRCULAR_EOD_STATUS> getTableObject(String p_businessDate,String p_instituteID,CohesiveSession session,DBSession dbSession,ReportDependencyInjection inject)throws DBProcessingException,DBValidationException{
        DirectoryStream<Path> stream =null;
        ArrayList<STUDENT_E_CIRCULAR_EOD_STATUS>totalEodList=new ArrayList();
        
        try{
        
        dbg("inside STUDENT_E_CIRCULAR_EOD_STATUS_DATASET--->getTableObject",session);    
...
//        
//        l_tableMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate, "BATCH", "STUDENT_E_CIRCULAR_EOD_STATUS", session, dbSession,ismaxVersionRequired);
//          
//                    
//        
//         dbg("end of STUDENT_E_CIRCULAR_EOD_STATUS_DATASET--->getTableObject",session);  
//        return   convertDBtoReportObject(l_tableMap,session) ;
        ...
        if(Files.notExists(ArchFolderPath)){
            ...
            for (Path file: stream) { 
             ...
             if(file.getFileName().toString().endsWith(fileExtension)){
                ...
                l_tableMap=readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+"ECircular"+i_db_properties.getProperty("FOLDER_DELIMITER")+fileNameWithoutExtension, "BATCH", "STUDENT_E_CIRCULAR_EOD_STATUS", session, dbSession,ismaxVersionRequired);
                ArrayList<STUDENT_E_CIRCULAR_EOD_STATUS>eodList=convertDBtoReportObject(l_tableMap,session);
         ...
         if(totalEodList.isEmpty()){
              ...
              ArrayList<STUDENT_E_CIRCULAR_EOD_STATUS>dataset=new ArrayList();
                STUDENT_E_CIRCULAR_EOD_STATUS appEod=new STUDENT_E_CIRCULAR_EOD_STATUS();
    ...
    private ArrayList<STUDENT_E_CIRCULAR_EOD_STATUS> convertDBtoReportObject(Map<String,DBRecord>l_tableMap,CohesiveSession session)throws DBProcessingException{
    
        ArrayList<STUDENT_E_CIRCULAR_EOD_STATUS>dataset=new ArrayList();
        try{
            
            
            dbg("inside STUDENT_E_CIRCULAR_EOD_STATUS_DATASET convertDBtoReportObject",session);
            ...
            if(l_tableMap!=null&&!l_tableMap.isEmpty()){
                ...
                while(recordIterator.hasNext()){
                    ...
                    STUDENT_E_CIRCULAR_EOD_STATUS eod=new STUDENT_E_CIRCULAR_EOD_STATUS();
            ...
            dbg("end of STUDENT_E_CIRCULAR_EOD_STATUS_DATASET convertDBtoReportObject",session);
