 ...
 * @author ibdtech
 */
public class TEACHER_E_CIRCULAR_EOD_STATUS_DATASET {
    public ArrayList<TEACHER_E_CIRCULAR_EOD_STATUS> getTableObject(String p_businessDate,String p_instituteID,CohesiveSession session,DBSession dbSession,ReportDependencyInjection inject)throws DBProcessingException,DBValidationException{
        DirectoryStream<Path> stream =null;
        ArrayList<TEACHER_E_CIRCULAR_EOD_STATUS>totalEodList=new ArrayList();
        
        try{
        
        dbg("inside TEACHER_E_CIRCULAR_EOD_STATUS_DATASET--->getTableObject",session);    
...
//        
//        l_tableMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate, "BATCH", "TEACHER_E_CIRCULAR_EOD_STATUS", session, dbSession,ismaxVersionRequired);
//          
//                    
//        
//         dbg("end of TEACHER_E_CIRCULAR_EOD_STATUS_DATASET--->getTableObject",session);  
//        return   convertDBtoReportObject(l_tableMap,session) ;
        ...
        if(Files.notExists(ArchFolderPath)){
            ...
            for (Path file: stream) { 
             ...
             if(file.getFileName().toString().endsWith(fileExtension)){
                ...
                l_tableMap=readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+"ECircular"+i_db_properties.getProperty("FOLDER_DELIMITER")+fileNameWithoutExtension, "BATCH", "TEACHER_E_CIRCULAR_EOD_STATUS", session, dbSession,ismaxVersionRequired);
                ArrayList<TEACHER_E_CIRCULAR_EOD_STATUS>eodList=convertDBtoReportObject(l_tableMap,session);
         ...
         if(totalEodList.isEmpty()){
              ...
              ArrayList<TEACHER_E_CIRCULAR_EOD_STATUS>dataset=new ArrayList();
                TEACHER_E_CIRCULAR_EOD_STATUS appEod=new TEACHER_E_CIRCULAR_EOD_STATUS();
    ...
    private ArrayList<TEACHER_E_CIRCULAR_EOD_STATUS> convertDBtoReportObject(Map<String,DBRecord>l_tableMap,CohesiveSession session)throws DBProcessingException{
    
        ArrayList<TEACHER_E_CIRCULAR_EOD_STATUS>dataset=new ArrayList();
        try{
            
            
            dbg("inside TEACHER_E_CIRCULAR_EOD_STATUS_DATASET convertDBtoReportObject",session);
            ...
            if(l_tableMap!=null&&!l_tableMap.isEmpty()){
                ...
                while(recordIterator.hasNext()){
                    ...
                    TEACHER_E_CIRCULAR_EOD_STATUS eod=new TEACHER_E_CIRCULAR_EOD_STATUS();
            ...
            dbg("end of TEACHER_E_CIRCULAR_EOD_STATUS_DATASET convertDBtoReportObject",session);
