 ...
 * @author DELL
 */
public class STUDENT_MARK_BATCH_STATUS_DATASET {
     public ArrayList<STUDENT_MARK_BATCH_STATUS> getTableObject(String p_businessDate,CohesiveSession session,DBSession dbSession,ReportDependencyInjection inject)throws DBProcessingException,DBValidationException{
        
        
        try{
        
        dbg("inside STUDENT_MARK_BATCH_STATUS_DATASET--->getTableObject",session);    
...
Map<String,DBRecord>l_tableMap=new HashMap();
        ...
        l_tableMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate, "BATCH", "STUDENT_MARK_BATCH_STATUS", session, dbSession,ismaxVersionRequired);
          
                    
        
         dbg("end of STUDENT_MARK_BATCH_STATUS_DATASET--->getTableObject",session);  
    ...
    private ArrayList<STUDENT_MARK_BATCH_STATUS> convertDBtoReportObject(Map<String,DBRecord>l_tableMap,CohesiveSession session)throws DBProcessingException{
    
        ArrayList<STUDENT_MARK_BATCH_STATUS>dataset=new ArrayList();
        try{
            
            
            dbg("inside STUDENT_MARK_BATCH_STATUS_DATASET convertDBtoReportObject",session);
           ...
           if(l_tableMap!=null&&!l_tableMap.isEmpty()){
                ...
                while(recordIterator.hasNext()){
                    ...
                    STUDENT_MARK_BATCH_STATUS eod=new STUDENT_MARK_BATCH_STATUS();
            ...
            dbg("end of STUDENT_MARK_BATCH_STATUS_DATASET convertDBtoReportObject",session);
