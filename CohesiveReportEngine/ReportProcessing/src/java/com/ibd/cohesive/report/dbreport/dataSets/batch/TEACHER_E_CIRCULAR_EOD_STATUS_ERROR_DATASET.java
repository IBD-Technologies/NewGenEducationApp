 ...
 * @author ibdtech
 */
public class TEACHER_E_CIRCULAR_EOD_STATUS_ERROR_DATASET {
    public ArrayList<TEACHER_E_CIRCULAR_EOD_STATUS_ERROR> getTableObject(String p_businessDate,String p_instituteID,CohesiveSession session,DBSession dbSession,ReportDependencyInjection inject)throws DBProcessingException,DBValidationException{
        
        DirectoryStream<Path> stream =null;
        ArrayList<TEACHER_E_CIRCULAR_EOD_STATUS_ERROR>totalEodList=new ArrayList();
        try{
        
        dbg("inside TEACHER_E_CIRCULAR_EOD_STATUS_ERROR_DATASET--->getTableObject",session);    
        ...
        if(Files.notExists(ArchFolderPath)){
            ...
            for (Path file: stream) { 
              ...
              try{  
                 ...
                 if(file.getFileName().toString().endsWith(fileExtension)){
                    ...
                    l_tableMap=readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+"ECircular"+i_db_properties.getProperty("FOLDER_DELIMITER")+fileNameWithoutExtension, "BATCH", "TEACHER_E_CIRCULAR_EOD_STATUS_ERROR", session, dbSession,ismaxVersionRequired);
                    ArrayList<TEACHER_E_CIRCULAR_EOD_STATUS_ERROR>eodList=convertDBtoReportObject(l_tableMap,session);
         ...
         if(totalEodList.isEmpty()){
             dbg("student fee eod status error empty",session);
              ArrayList<TEACHER_E_CIRCULAR_EOD_STATUS_ERROR>dataset=new ArrayList();
                TEACHER_E_CIRCULAR_EOD_STATUS_ERROR appEod=new TEACHER_E_CIRCULAR_EOD_STATUS_ERROR();
         ...
         dbg("end of TEACHER_E_CIRCULAR_EOD_STATUS_ERROR_DATASET--->getTableObject",session);  
        return   totalEodList ;
    ...
    private ArrayList<TEACHER_E_CIRCULAR_EOD_STATUS_ERROR> convertDBtoReportObject(Map<String,DBRecord>l_tableMap,CohesiveSession session)throws DBProcessingException{
    
       ArrayList<TEACHER_E_CIRCULAR_EOD_STATUS_ERROR> dataset=new ArrayList();
        try{
            
            
            dbg("inside TEACHER_E_CIRCULAR_EOD_STATUS_ERROR_DATASET convertDBtoReportObject",session);
            ...
            if(l_tableMap!=null&&!l_tableMap.isEmpty()){
                ...
                while(recordIterator.hasNext()){
                    ...
                    TEACHER_E_CIRCULAR_EOD_STATUS_ERROR eod=new TEACHER_E_CIRCULAR_EOD_STATUS_ERROR();
            ...
            dbg("end of TEACHER_E_CIRCULAR_EOD_STATUS_ERROR_DATASET convertDBtoReportObject",session);
