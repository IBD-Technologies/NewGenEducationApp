 ...
 * @author DELL
 */
public class APP_EOD_STATUS_DATASET {
//    ArrayList<APP_EOD_STATUS> dataset;
    
    
    public ArrayList<APP_EOD_STATUS> getTableObject(String p_businessDate,CohesiveSession session,DBSession dbSession,ReportDependencyInjection inject)throws DBProcessingException,DBValidationException{
        
        
        try{
        
        dbg("inside APP_EOD_STATUS_DATASET--->getTableObject",session);    
        ...
        Map<String,DBRecord>l_appStatusMap=null;
        
        try{
        
          l_appStatusMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate, "BATCH", "APP_EOD_STATUS", session, dbSession,ismaxVersionRequired);
          
        }catch(DBValidationException ex){
            
            if(ex.toString().contains("DB_VAL_011")||ex.toString().contains("DB_VAL_000")){
                
                ArrayList<APP_EOD_STATUS>dataset=new ArrayList();
                APP_EOD_STATUS appEod=new APP_EOD_STATUS();
         ...
         dbg("end of APP_EOD_STATUS_DATASET--->getTableObject",session);  
    ...
    private ArrayList<APP_EOD_STATUS> convertDBtoReportObject(Map<String,DBRecord>p_appEodMap,CohesiveSession session)throws DBProcessingException{
    
        ArrayList<APP_EOD_STATUS>dataset=new ArrayList();
        try{
            
            
            dbg("inside APP_EOD_STATUS_DATASET convertDBtoReportObject",session);
            ...
            dbg("end of APP_EOD_STATUS_DATASET convertDBtoReportObject",session);
