 ...
 * @author DELL
 */
public class FEE_EOD_STATUS_HISTORY_DATASET {
      public ArrayList<FEE_EOD_STATUS_HISTORY> getTableObject(String p_businessDate,String p_instituteID,CohesiveSession session,DBSession dbSession,ReportDependencyInjection inject)throws DBProcessingException,DBValidationException{
        
        
        try{
        
        dbg("inside FEE_EOD_STATUS_HISTORY_DATASET--->getTableObject",session);    
      ...
      try{
        ...
        l_tableMap=readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate, "BATCH", "FEE_EOD_STATUS_HISTORY", session, dbSession,ismaxVersionRequired);
          
        }catch(DBValidationException ex){
            
            if(ex.toString().contains("DB_VAL_011")||ex.toString().contains("DB_VAL_000")){
                
                ArrayList<FEE_EOD_STATUS_HISTORY>dataset=new ArrayList();
                FEE_EOD_STATUS_HISTORY appEod=new FEE_EOD_STATUS_HISTORY();
         ...
         dbg("end of FEE_EOD_STATUS_HISTORY_DATASET--->getTableObject",session);  
    ...
    private ArrayList<FEE_EOD_STATUS_HISTORY> convertDBtoReportObject(Map<String,DBRecord>l_tableMap,CohesiveSession session)throws DBProcessingException{
    
       ArrayList<FEE_EOD_STATUS_HISTORY> dataset=new ArrayList();
        try{
            
            
            dbg("inside FEE_EOD_STATUS_HISTORY_DATASET convertDBtoReportObject",session);
            ...
            if(l_tableMap!=null&&!l_tableMap.isEmpty()){
                ...
                while(recordIterator.hasNext()){
                    ...
                    FEE_EOD_STATUS_HISTORY eod=new FEE_EOD_STATUS_HISTORY();
            ...
            dbg("end of FEE_EOD_STATUS_HISTORY_DATASET convertDBtoReportObject",session);
