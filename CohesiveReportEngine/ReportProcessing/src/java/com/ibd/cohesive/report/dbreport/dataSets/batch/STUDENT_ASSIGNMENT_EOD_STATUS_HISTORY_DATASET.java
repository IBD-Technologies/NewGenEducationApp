 ...
 * @author DELL
 */
public class STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY_DATASET {
 public ArrayList<STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY> getTableObject(String p_businessDate,String p_instituteID,CohesiveSession session,DBSession dbSession,ReportDependencyInjection inject)throws DBProcessingException,DBValidationException{
        
        DirectoryStream<Path> stream =null;
        ArrayList<STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY>totalEodList=new ArrayList();
        try{
        
        dbg("inside STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY_DATASET--->getTableObject",session);    
        ...
        if(Files.notExists(ArchFolderPath)){
            ...
            try{    
                 ...
                 if(file.getFileName().toString().endsWith(fileExtension)){
                    ...
                    l_tableMap=readBuffer.readTable("INSTITUTE"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_instituteID+i_db_properties.getProperty("FOLDER_DELIMITER")+"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+"BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+"Assignment"+i_db_properties.getProperty("FOLDER_DELIMITER")+fileNameWithoutExtension, "BATCH", "STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY", session, dbSession,ismaxVersionRequired);
                    ArrayList<STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY>eodList=convertDBtoReportObject(l_tableMap,session);
         ...
         if(totalEodList.isEmpty()){
             ...
             dbg("student otherActivity eod status history empty",session);
              ArrayList<STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY>dataset=new ArrayList();
                STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY appEod=new STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY();
    ...
    private ArrayList<STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY> convertDBtoReportObject(Map<String,DBRecord>l_tableMap,CohesiveSession session)throws DBProcessingException{
    
        ArrayList<STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY>dataset=new ArrayList();
        try{
            
            
            dbg("inside STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY_DATASET convertDBtoReportObject",session);
            ...
            if(l_tableMap!=null&&!l_tableMap.isEmpty()){
                ...
                while(recordIterator.hasNext()){
                    ...
                    STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY eod=new STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY();
            ...
            dbg("end of STUDENT_ASSIGNMENT_EOD_STATUS_HISTORY_DATASET convertDBtoReportObject",session);
