 ...
 * @author DELL
 */
public class INSTITUTE_EOD_STATUS_DATASET {
//    ArrayList<INSTITUTE_EOD_STATUS> dataset;
    
    
    public ArrayList<INSTITUTE_EOD_STATUS> getTableObject(String p_businessDate,CohesiveSession session,DBSession dbSession,ReportDependencyInjection inject)throws DBProcessingException,DBValidationException{
        
        
        try{
        
        dbg("inside INSTITUTE_EOD_STATUS_DATASET--->getTableObject",session);    
...
Map<String,DBRecord>l_tableMap=null;
        ...
        try{
          ...
          l_tableMap=readBuffer.readTable("BATCH"+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate+i_db_properties.getProperty("FOLDER_DELIMITER")+p_businessDate, "BATCH", "INSTITUTE_EOD_STATUS", session, dbSession,ismaxVersionRequired);
          
          }catch(DBValidationException ex){
            
            if(ex.toString().contains("DB_VAL_011")||ex.toString().contains("DB_VAL_000")){
                
                ArrayList<INSTITUTE_EOD_STATUS>dataset=new ArrayList();
                INSTITUTE_EOD_STATUS appEod=new INSTITUTE_EOD_STATUS();
         ...
         dbg("end of INSTITUTE_EOD_STATUS_DATASET--->getTableObject",session);  
    ...
    private ArrayList<INSTITUTE_EOD_STATUS> convertDBtoReportObject(Map<String,DBRecord>l_tableMap,CohesiveSession session)throws DBProcessingException{
    
        ArrayList<INSTITUTE_EOD_STATUS>dataset=new ArrayList();
        try{
            
            
            dbg("inside INSTITUTE_EOD_STATUS_DATASET convertDBtoReportObject",session);
            ...
            if(l_tableMap!=null&&!l_tableMap.isEmpty()){
                ...
                while(recordIterator.hasNext()){
                    ...
                    INSTITUTE_EOD_STATUS eod=new INSTITUTE_EOD_STATUS();
            ...
            dbg("end of INSTITUTE_EOD_STATUS_DATASET convertDBtoReportObject",session);
